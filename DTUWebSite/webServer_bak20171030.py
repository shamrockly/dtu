...
class dtu1001_page1:
    def GET(self):
        ...
        if(len(dtuinfo)==0):
            ...
            active_power_1=get_real_value(float(queryres['active_power_hign_1'])*65536+float(queryres['active_power_low_1']), 10)
            reactive_power_1=get_real_value(float(queryres['reactive_power_high_1'])*65536+float(queryres['reactive_power_low_1']), 10)
            power_factor_1=get_real_value_h(queryres['power_factor_1'], 100)

            voltage_2=queryres['voltage_2']
            current_2=queryres['current_2']
            active_power_2=get_real_value(float(queryres['active_power_hign_2'])*65536+float(queryres['active_power_low_2']), 10)
            reactive_power_2=get_real_value(float(queryres['reactive_power_high_2'])*65536+float(queryres['reactive_power_low_2']), 10)
            power_factor_2=get_real_value_h(queryres['power_factor_2'], 100)

            voltage_3=queryres['voltage_3']
            current_3=queryres['current_3']
            active_power_3=get_real_value(float(queryres['active_power_hign_3'])*65536+float(queryres['active_power_low_3']), 10)
            reactive_power_3=get_real_value(float(queryres['reactive_power_high_3'])*65536+float(queryres['reactive_power_low_3']), 10)
...
class dtu1001_page2:
    def GET(self):
        ...
        eventhistory = config.DB.query('select * from dtu_event order by time;')

        historyinfo = config.DB.query('select * from dtu_table1v2 order by updatetime desc limit 1;');
        if(len(historyinfo)==0):
            resultarray = [-1 for i in range(0,33)]
            return render1.page2(resultarray,eventhistory,dtuid)

        queryres = historyinfo[0]
        resultarray = [] 
        ...
        flag_switch_mcr_1 = queryres['flag_switch_mcr_1']
        flag_switch_mcr_2 = queryres['flag_switch_mcr_2']
        flag_lock_status = queryres['flag_lock_status']		
        flag_null_status = queryres['flag_null_status']		
        flag_voltage_over_1 = queryres['flag_voltage_over_1']
        flag_voltage_under_1 = queryres['flag_voltage_under_1']
        flag_threshold_over_1 = queryres['flag_threshold_over_1']
        flag_current_over_1 = queryres['flag_current_over_1']
        flag_current_unblance_1 = queryres['flag_current_unblance_1']
        flag_sampling_error_1 = queryres['flag_sampling_error_1']
        flag_sampling_warning_1 = queryres['flag_sampling_warning_1']
        flag_switch_gas_1 = queryres['flag_switch_gas_1']
        flag_prediction_1 = queryres['flag_prediction_1']
        flag_high_temperature_1 = queryres['flag_high_temperature_1']
        flag_voltage_over_2 = queryres['flag_voltage_over_2']
        flag_voltage_under_2 = queryres['flag_voltage_under_2']
        flag_threshold_over_2 = queryres['flag_threshold_over_2']
        flag_current_over_2 = queryres['flag_current_over_2']
        flag_current_unblance_2 = queryres['flag_current_unblance_2']
        flag_sampling_error_2 = queryres['flag_sampling_error_2']
        flag_sampling_warning_2 = queryres['flag_sampling_warning_2']
        flag_switch_gas_2 = queryres['flag_switch_gas_2']
        flag_prediction_2 = queryres['flag_prediction_2']
        flag_high_temperature_2 = queryres['flag_high_temperature_2']
        flag_magnetic_reset = queryres['flag_magnetic_reset']	
        flag_power_factor_1 = queryres['flag_power_factor_1']	
        flag_power_factor_2 = queryres['flag_power_factor_2']	
        flag_entry_error_1 = queryres['flag_entry_error_1']		
        flag_entry_error_2 = queryres['flag_entry_error_2']	

        resultarray.append(str(updatetime)) # 0
        resultarray.append(flag_switch_main)        
        resultarray.append(flag_switch_entry_1)     
        resultarray.append(flag_switch_entry_2)     
        resultarray.append(flag_switch_mcr_1)       
        resultarray.append(flag_switch_mcr_2)       
        resultarray.append(flag_lock_status)        
        resultarray.append(flag_null_status)        
        resultarray.append(flag_voltage_over_1)     
        resultarray.append(flag_voltage_under_1)    
        resultarray.append(flag_threshold_over_1)  # 10 
        resultarray.append(flag_current_over_1)     
        resultarray.append(flag_current_unblance_1) 
        resultarray.append(flag_sampling_error_1)   
        resultarray.append(flag_sampling_warning_1) 
        resultarray.append(flag_switch_gas_1)       
        resultarray.append(flag_prediction_1)       
        resultarray.append(flag_high_temperature_1) 
        resultarray.append(flag_voltage_over_2)     
        resultarray.append(flag_voltage_under_2)    
        resultarray.append(flag_threshold_over_2) # 20  
        resultarray.append(flag_current_over_2)     
        resultarray.append(flag_current_unblance_2) 
        resultarray.append(flag_sampling_error_2)   
        resultarray.append(flag_sampling_warning_2) 
        resultarray.append(flag_switch_gas_2)       
        resultarray.append(flag_prediction_2)       
        resultarray.append(flag_high_temperature_2) 
        resultarray.append(flag_magnetic_reset)     
        resultarray.append(flag_power_factor_1)     
        resultarray.append(flag_power_factor_2)  # 30   
        resultarray.append(flag_entry_error_1)      
        resultarray.append(flag_entry_error_2)
        return render1.page2(resultarray,eventhistory,dtuid)
...
class dtu1001_page3:
    def GET(self):
        ...
        updatetime_array=[]
        for item in result:
            power_factor_1_array.append(get_real_value_h(float(item['power_factor_1']), 100))
            power_factor_2_array.append(get_real_value_h(float(item['power_factor_2']), 100))
            power_factor_3_array.append(get_real_value_h(float(item['power_factor_3']), 100))
            voltage_1_array.append(float(item['voltage_1']))
            voltage_2_array.append(float(item['voltage_2']))
            voltage_3_array.append(float(item['voltage_3']))
            current_1_array.append(float(item['current_1']))
            current_2_array.append(float(item['current_2']))
            current_3_array.append(float(item['current_3']))
            mcr_voltage_A_1_array.append(float(item['mcr_voltage_A_1']))
            mcr_voltage_B_1_array.append(float(item['mcr_voltage_B_1']))
            mcr_voltage_C_1_array.append(float(item['mcr_voltage_C_1']))
            mcr_voltage_A_2_array.append(float(item['mcr_voltage_A_2']))
            mcr_voltage_B_2_array.append(float(item['mcr_voltage_B_2']))
            mcr_voltage_C_2_array.append(float(item['mcr_voltage_C_2']))
            mcr_current_A_1_array.append(float(item['mcr_current_A_1']))
            mcr_current_B_1_array.append(float(item['mcr_current_B_1']))
            mcr_current_C_1_array.append(float(item['mcr_current_C_1']))
            mcr_current_A_2_array.append(float(item['mcr_current_A_2']))
            mcr_current_B_2_array.append(float(item['mcr_current_B_2']))
            mcr_current_C_2_array.append(float(item['mcr_current_C_2']))
...
class dtu1001_page33:
    def GET(self):
        ...
        for item in result:
            power_factor_1_array.append(get_real_value_h(float(item['power_factor_1']), 100))
            power_factor_2_array.append(get_real_value_h(float(item['power_factor_1']), 100))
            power_factor_3_array.append(get_real_value_h(float(item['power_factor_1']), 100))
            voltage_1_array.append(float(item['voltage_1']))
            voltage_2_array.append(float(item['voltage_2']))
            voltage_3_array.append(float(item['voltage_3']))
            current_1_array.append(float(item['current_1']))
